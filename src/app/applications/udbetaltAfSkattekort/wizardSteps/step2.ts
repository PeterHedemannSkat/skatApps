...
export class basicSkattekort {
  ...
  ngOnInit() {
    ...
    this.thisincome = this.wizardState.incomes[
      this.wizardState.thisincome
    ].income;
    ...
    this.wizardState.printLocalContent([
      ["step2", "general"],
      ["general", "placeholderTexts"]
    ]);
    this.wizardState.printLocalLists([
      "loonindkomstTyper",
      "skattekortsTyper",
      "indkomstperioder"
    ]);
  ...
  correctIncomeHelperTxt() {
    return this.thisincome.type === "loonIndkomst"
      ? this.content["helpIndkomst"]
      : this.content["helpOtherThanSalery"];
  }

  getNumberType() {
    return [
      this.wizardState.errorTxt["notEmpty"],
      this.wizardState.errorTxt["number"]
    ];
  }
  ...
  validate() {
    ...
    var validates = [this.thisincome.sum].map(el => {
      return { element: el, regExContainer: numbers };
    });

    this.wizardState.stepValidationOk = new Validator()
      .add(validates)
      .checkAll();
  }
  ...
  headingDynamic() {
    return !this.wizardState.editingIncome
      ? this.content["headingNy"]
      : this.content["headingEdit"];
  }
  ...
  monthSaleryTextTotal(): string {
    var period = this.thisincome.period,
      id =
        period === "monthly"
          ? "indkomstLabelDel2Monthly"
          : "indkomstLabelDel2TwoWeekly",
  ...
  updateskattekort() {
    ...
    let hovedkortExists = this.wizardState.incomes.reduce((p, v, i) => {
      return v.income.appliedTaxCard === "hovedkort" &&
        i !== indexOfCurrentIncome
        ? i
        : p;
    }, -1);
    ...
    if (hovedkortExists > -1) {
      ...
      this.previousHovedkort =
        typePrev !== "loonIndkomst"
          ? this.indkomstTyper[typePrev]
          : this.wizardState.incomes[hovedkortExists].income.from;
      this.newHovedkort =
        typeNow !== "loonIndkomst"
          ? this.indkomstTyper[typeNow]
          : this.thisincome.from;

      this.hovedkortchange = true;
      this.wizardState.moveHovedkort(
        hovedkortExists,
        this.wizardState.thisincome
      );
    } else {
  ...
  changePeriod() {
    if (this.thisincome.type !== "loonIndkomst")
      this.thisincome.period = "monthly";
  }
}
