...
import { Validator, validateSet, regMapElement } from '../services/validator.service';
        ...
        <div class="row skts-process-form-section form-group">
            ...
            <div class = "col-sm-6">
                ...
                <div class="input-group">
                    <input [(ngModel)] = "value" class="form-control" (focus) = "errors = []"  (keyup) = "inputChanged($event)" (blur) = "blur()" [attr.placeholder] = "placeholder" #spy = "ngModel">
                    <div class="input-group-append" *ngIf="!!postfix">
                        <span class="input-group-text align-units">{{postfix}}</span>
                    </div>
                </div>
                <div [hidden] = "errors.length === 0 || (spy.pristine)" class = "text-danger" [class.show] = "errors.length > 0">{{printMostRelevantError()}}</div>
            </div>
        </div>
